...
namespace Persistence.PostgreSql.IntegrationTests
{
    public class PostgreSqlEventStreamStagingTestReadRepository : IEventStreamStagingTestReadRepository
    {
        ...
        public async Task<IReadOnlyList<EventStreamStagingEntryTestReadModel>> SelectAsync(Guid stagingId)
        {
            ...
            command.CommandText = "SELECT StagingId, StreamId, EntrySequence, EntryId, EventContent, EventContentSerializationFormat, EventTypeIdentifier, EventTypeIdentifierFormat, CausationId, CreationTime, CreationTimeNanoSeconds, CorrelationId FROM EventStreamStaging WHERE StagingId = @StagingId";

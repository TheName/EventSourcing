    ...
    public class PostgreSqlEventStreamTestReadRepository : IEventStreamTestReadRepository
    {
        ...
        public async Task<IReadOnlyList<EventStreamEntryTestReadModel>> SelectAsync(Guid streamId)
        {
            using (var connection = new NpgsqlConnection(_configuration.ConnectionString))
            {
                ...
                {
                    command.CommandText =
                        "SELECT StreamId, EntrySequence, EntryId, EventContent, EventContentSerializationFormat, EventTypeIdentifier, EventTypeIdentifierFormat, CausationId, CreationTime, CreationTimeNanoSeconds, CorrelationId FROM EventStream WHERE StreamId = @StreamId";

    ...
    public class SqlServerEventStreamStagingTestRepository : IEventStreamStagingTestRepository
    {
        ...
        public async Task<IReadOnlyList<EventStreamStagingEntryTestReadModel>> SelectAsync(Guid stagingId)
        {
            using (var connection = new SqlConnection(_configuration.ConnectionString))
            {
                ...
                {
                    command.CommandText =
                        "SELECT StagingId, StreamId, EntrySequence, EntryId, EventContent, EventContentSerializationFormat, EventTypeIdentifier, EventTypeIdentifierFormat, CausationId, CreationTime, CorrelationId FROM EventStreamStaging WHERE StagingId = @StagingId";
